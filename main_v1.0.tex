...
$2\times3$ and $3\times3$; $|S|$ is invariant under $24$ $O(4)$ gauge
...
the full signed $S$ and $T$ from the lattice on \emph{two} sizes, circularity-
free, up to the $\mathbb{Z}_2$ relabeling intrinsic to an achiral theory; a
validated two-channel error budget with no free parameters accounts for the
extraction residual on both. The modular data themselves are not new with this
...
\begin{equation}
  ...
  \label{eq:target}
\end{equation}
with $\varphi=(1+\sqrt5)/2$ the golden ratio. This quantity is the heart of
topological quantum computation. Obtaining it \emph{Hamiltonian-dynamically},
...
size, $\lVert[M_i,M_j]\rVert = 5.6\times10^{-2}$, this noncommutativity lifts
...
precision. The theory-free Verlinde--Casimir route reproduces the analytic
...
$\{0,\pm4\pi/5\}$. From the loop overlaps \emph{alone} they are fixed only
...
\item[(b2)] a \emph{Gauss-sum/anomaly} argument~\cite{Vafa1988,Bantay1997}:
$p_{+}=\sum_a d_a^{2}\theta_a = D$ at $c=0$ pins the solution uniquely without
invoking $(ST)^{3}$, and rejects the order-3 alternative ($5.236 \ne D$). The
order-5 congruence together with $c=0$ then selects the physical rather than
the Galois-conjugate solution.
\end{enumerate}
...
$O(4)$ gauge transformations of the ground-state manifold, with a worst
...
vanish identically ($\lVert i(M-M^{\mathsf T})/2\rVert \approx 1.2\times10^{-16}$
...
$6.4\times10^{-10}$ to $1.8\times10^{-9}$, again with correct sign structure;
there the three centers are distinct as deposited objects, and we expect, but
have not verified, the same center independence, since the $2\times2$ basis is
not part of this record. The full signed $S$ and $T$ are thereby resolved from
...
The extraction residual is not a single number but a two-channel budget, and
validating it across both lattice sizes with no free parameters is a result in
its own right. The residual follows
\begin{equation}
...
\quad Verlinde--Casimir vs.\ theory & $1.1\times10^{-16}$ & (a) one route, theory-free & $3\times3$ \\
...
\colrule
Topological spins $T$ & $\{0,\pm4\pi/5\}$; Verlinde resid.\ $3.98\times10^{-11}$ & (b) consistency; measured in (d) & lattice $S$ \\
...
networks~\cite{ConlonSlingerland2022,Maciazek2023}, in which a network that is
not triconnected does not support planar encircling braiding.
...
$\sqrt2=\lVert\sigma_y\rVert$, and the monodromy $[s_1^{2},s_2^{2}]$ has
Frobenius norm $2\sqrt2=\lVert2\sigma_y\rVert$. These are properties of the
...
On the hardware side, non-Abelian braiding and Fibonacci statistics have been
realized as digital gate sequences~\cite{Xu2024,Andersen2023,Iqbal2024}, and
...
\href{https://doi.org/10.5281/zenodo.21362246}{doi:10.5281/zenodo.21362246}
...
\texttt{p6\_modular\_reference.py} rebuilds from scratch, and the driver script
performing that step is not part of this record; nor is the generation of the
matrices themselves, since the exact-diagonalization engine that produces the
ground-state and loop matrices is a separate, heavier computation.
